...
export const AgentInteraction = ({ userData, jobs, setJobs, selectedJob, setSelectedJob, connectedApps, setConnectedApps, nonConnectedApps, setNonConnectedApps }) => {
    const { id } = useParams();
    ...
    const sendMessage = async (input_message = null) => {
        ...
        if (input_message.trim() !== '' || attachedImages.length > 0 || attachedDocuments.length > 0) { // Adjusted condition
            ...
            if (!user && !firstMessage) { // what if we restrict this to the SECOND message, not the first
            ...
            socket.onmessage = (message) => {
                try {
                    ...
                    if (transcript) {
                        ...
                        const currentValue = currentInputMessageRef.current;
                        const newInputMessage = currentValue + " " + transcript;
    ...
    // Keep the ref updated with the current value of inputMessage
    useEffect(() => {
        currentInputMessageRef.current = inputMessage;
    }, [inputMessage]);




    // Filtering which messages to show in the chat

    const filteredMessages = messages.filter((message, index) => {
        if (index < messages.length - 1 && (messages[index + 1].type === 'mass_content_generation' || messages[index + 1].type === 'research')) {
            return false; // remove the message right before a mass_content_generation message since that's just the "Writing" message which isn't relevant anymore
        }
        return true;
    });
    ...
    return (
        <div className='agent_container'>
            <AnimatePresence>
                {showBrowsingPopup && browsingURL && browsingActionRect && (
                    <motion.div
                        className="browsing-popup-overlay"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        onClick={() => setShowBrowsingPopup(false)}
                    >
                        <motion.div
                            className="browsing-popup"
                            initial={{
                                y: browsingActionRect.y - 100,
                                width: browsingActionRect.width,
                                height: browsingActionRect.height,
                                opacity: 0
                            }}
                            animate={{
                                width: 1280,
                            ...
                            exit={{
                                x: browsingActionRect.x,
                                y: browsingActionRect.y,
                                width: browsingActionRect.width,
                                height: browsingActionRect.height,
                                opacity: 0
                            }}
                        ...
                        >
                            <button 
                                onClick={() => setShowBrowsingPopup(false)}
                                className="browsing_close_button"
                                style={{zIndex: 3}}
                            >
                                <IoClose size={24} />
                            </button>
                            <div className="popup-content" style={{height: '100%', width: '100%', position: 'relative'}}>
                                <div 
                                    style={{
                                        position: 'absolute',
                                        top: '50%',
                                ...
                                <iframe
                                    ...
                                    style={{ width: "100%", height: "100%", pointerEvents: 'none', zIndex: 2, borderRadius: '8px'}}
                                    allowFullScreen
                                />
                            </div>
                        </motion.div>
                            ...
                            <div className="chat_intro_area">
                                ...
                                <div className="chat_intro_area_examples">
                                    ...
                                    <div className="examples_container" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '10px' }}>
                                        ...
                                        {/* <div className="example_card" onClick={() => {
                                            setInputMessage("Get the latest posts on r/AskReddit");
                                            sendMessage("Get the latest posts on r/AskReddit");
                                        }}>
                                            <div className="category_overlay">Information</div>
                                            <p>Get the latest posts on r/AskReddit</p>
                                        </div> */}
                                        <div className="example_card" onClick={() => {
                                        ...
                                        {/* <div className="example_card" onClick={() => {
                                            setInputMessage("Create a new GitHub repository called \"AGI\"");
                                            sendMessage("Create a new GitHub repository called \"AGI\"");
                                        }}>
                                            <div className="category_overlay">Development</div>
                                            <p>Create a new GitHub repository called "AGI"</p>
                                        </div> */}
                                    </div>
                                </div>
                            </div>
